...
type MatcherService interface {
	...
	MatchSections(rawSections []models.StudentSections, section *models.R)
}
...
func (m *matcherService) MatchSections(rawSections []models.StudentSections, section *models.R) {
	...
	for _, sectionData := range rawSections {
		...
		for _, class := range sectionData.AllSectionsJSON.Edges {
			...
			if _, ok := tmpSection[code]; !ok {
				tmp := models.Section{}
				tmp.Node = class.Node
				tmpSection[code] = tmp
			}
			tmpS := tmpSection[code]
			tmpS.Participants = append(tmpS.Participants, sectionData.StudentsJSON.Name)
			tmpSection[code] = tmpS
		}
	}
	section.Students = students
	section.Sections = tmpSection
}
